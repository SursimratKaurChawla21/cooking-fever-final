...
function setup(){
  ...
  food3.scale=0.5
  food3.visible=false
  list1=createSprite(300,320)
  list1.visible=false
  list2=createSprite(300,320)
  list2.visible=false
  list3=createSprite(300,320)
  list3.visible=false
...
function draw(){
  ...
  if(gameState===0){
        textSize(20)
        fill("white")
        text("Click on start to choose the recipe",150,350)
        text("BASKET CONTROLS: -> To move Right",100,450)
        text(" <-To move Left",300,490)
        button.mousePressed(()=> {
                input.hide()
                button.hide()
                var name = input.value()
                greet.html("Hi "+ name + " Choose your recipe")
                greet.position(150,100)
                greet.class("greet")
                food1.visible = true
                food2.visible = true
                food3.visible = true
...
if(gameState===1){
        textSize(20)
        fill("cyan")
        text("You have time limit to go through the ingredients",100,500)
...
if(gameState===2){
        ...
        fill("white")
        textSize(20)
        text("Timer : "+timer,480,100)
        fill("white")
        textSize(20)
        text("Score : "+score,480,130)
        if(timer<=0){
                timer=0
                gameState=3
        }
...
if(gameState===3){
          ...
          ingredients1Group.destroyEach()
          ingredients2Group.destroyEach()
          fill("white")
          stroke("black")
          strokeWeight(3)
          textSize(20)
          if(score>15){
                ...
                text("You have collected the ingredients successfully",100,300)
          }
          else{
                text("GAME ENDED",200,200)
                text("You have not collected all the ingredients",100,300)   
          }
}
